...
public class LocalVariableSymbol extends BaseSymbol {
  ...
  public void updateCodeZone(CodeZone cz) {
    if(this.cz.beginLine() < cz.beginLine() || 
        (this.cz.beginLine() == cz.beginLine() && this.cz.beginColumn() < cz.beginColumn())) {
      this.cz = new CodeZoneClass(cz, this.cz);
    }
    if(this.cz.endLine() > cz.endLine() || 
        (this.cz.endLine() == cz.endLine() && this.cz.endColumn() > cz.endColumn())) {
